...
void ERRORFORTYPE(const PString *MSG, TPSTYPE *GOT) {
  COMPILEERROR(CONCAT(CpString, *MSG, CpLenPtr, 6, "; got ", CpEnd | CpString, TYPENAME(GOT)));
}

void ERRORFOREXPR(const PString *MSG, TEXPRESSIONOBJ *GOT) {
  COMPILEERROR(CONCAT(CpString, *MSG, CpLenPtr, 6, "; got ", CpEnd | CpString, ERRORDESCRIBEEXPR(GOT)));
}
...
TPSNAME *_FINDNAME(const PString *NAME, PBoolean REQUIRED, PBoolean FROMLOCALSCOPE) {
  ...
  while (RET == (void*)0 && DEF != (void*)0 && (!FROMLOCALSCOPE || DEF->CLS != TDCSCOPEBOUNDARY)) {
    if (DEF->CLS == TDCNAME && cmp_str(CoEq, CpString, *NAME, CpStringPtr, &DEF->NAMEPTR->NAME)) RET = DEF->NAMEPTR;
    DEF = DEF->PREV;
  }
  if (REQUIRED && RET == (void*)0) COMPILEERROR(CONCAT(CpLenPtr, 20, "Unknown identifier: ", CpEnd | CpString, *NAME));
  RESULT = RET;
  return RESULT;
}
...
TPSNAME *FINDNAMEINLOCALSCOPE(const PString *NAME, PBoolean REQUIRED) {
  TPSNAME *RESULT;
  RESULT = ({ PString tmp1 = *NAME; _FINDNAME(&tmp1, REQUIRED, 1); });
  return RESULT;
}

TPSNAME *FINDNAMEOFCLASSINLOCALSCOPE(const PString *NAME, TPSNAMECLASS CLS, PBoolean REQUIRED) {
  TPSNAME *RESULT;
  RESULT = _CHECKNAMECLASS(({ PString tmp1 = *NAME; FINDNAMEINLOCALSCOPE(&tmp1, REQUIRED); }), CLS);
  return RESULT;
}

TPSNAME *FINDNAME(const PString *NAME, PBoolean REQUIRED) {
  TPSNAME *RESULT;
  RESULT = ({ PString tmp1 = *NAME; _FINDNAME(&tmp1, REQUIRED, 0); });
  return RESULT;
}

TPSNAME *FINDNAMEOFCLASS(const PString *NAME, TPSNAMECLASS CLS, PBoolean REQUIRED) {
  TPSNAME *RESULT;
  RESULT = _CHECKNAMECLASS(({ PString tmp1 = *NAME; FINDNAME(&tmp1, REQUIRED); }), CLS);
  return RESULT;
}
...
TPSNAME *_ADDNAME(const PString *NAME, TPSNAMECLASS CLS) {
  TPSNAME *RESULT;
  TPSNAME *POS;
  if (({ PString tmp1 = *NAME; FINDNAMEINLOCALSCOPE(&tmp1, 0); }) != (void*)0) COMPILEERROR(CONCAT(CpLenPtr, 11, "Identifier ", CpString, *NAME, CpEnd | CpLenPtr, 16, " already defined"));
...
TPSNAME *ADDTYPENAME(const PString *NAME, TPSTYPE *IDX) {
  TPSNAME *RESULT;
  TPSNAME *DEF;
  DEF = ({ PString tmp1 = *NAME; _ADDNAME(&tmp1, TNCTYPE); });
  DEF->TYPEPTR = IDX;
  RESULT = DEF;
  return RESULT;
}
...
TPSNAME *ADDVARIABLENAME(const PString *NAME, TPSVARIABLE *IDX) {
  TPSNAME *RESULT;
  TPSNAME *DEF;
  DEF = ({ PString tmp1 = *NAME; _ADDNAME(&tmp1, TNCVARIABLE); });
  DEF->VARPTR = IDX;
  RESULT = DEF;
  return RESULT;
}
...
TPSNAME *ADDCONSTANTNAME(const PString *NAME, TPSCONSTANT *IDX) {
  TPSNAME *RESULT;
  TPSNAME *DEF;
  DEF = ({ PString tmp1 = *NAME; _ADDNAME(&tmp1, TNCCONSTANT); });
  DEF->CONSTPTR = IDX;
  RESULT = DEF;
  return RESULT;
}
...
TPSNAME *ADDFUNCTIONNAME(const PString *NAME, TPSFUNCTION *IDX) {
  TPSNAME *RESULT;
  TPSNAME *DEF;
  DEF = ({ PString tmp1 = *NAME; _ADDNAME(&tmp1, TNCFUNCTION); });
  DEF->FNPTR = IDX;
  RESULT = DEF;
  return RESULT;
}
...
TPSNAME *ADDPSEUDOFN(const PString *NAME, TPSPSEUDOFN FN) {
  TPSNAME *RESULT;
  TPSNAME *DEF;
  DEF = ({ PString tmp1 = *NAME; _ADDNAME(&tmp1, TNCPSEUDOFN); });
  DEF->PSEUDOFN = FN;
  RESULT = DEF;
  return RESULT;
}
...
PString UNPARSESTRING(const PString *ST) {
  ...
  do {
    PInteger first = 1;
    PInteger last = ({ PString tmp1 = *ST; LENGTH(&tmp1); });
    if (first <= last) {
      POS = first;
      while (1) {
        {
...
PBoolean HASFORWARDDECLARATION(const PString *NAME) {
  PBoolean RESULT;
  TPSNAME *NAMEPTR;
  NAMEPTR = ({ PString tmp1 = *NAME; FINDNAMEOFCLASSINLOCALSCOPE(&tmp1, TNCFUNCTION, 0); });
...
PInteger FINDFIELD(TPSTYPE *TYPEPTR, const PString *NAME, PBoolean REQUIRED) {
  ...
  ENSURERECORDTYPE(TYPEPTR);
  {
    TPSRECORDDEF *with1 = TYPEPTR->RECPTR;
    {
      ...
      POS = with1->SIZE;
      while (POS >= 1 && RET == 0) {
        if (cmp_str(CoEq, CpString, *NAME, CpStringPtr, &with1->FIELDS[subrange(POS, 1, 32) - 1].NAME)) RET = POS;
        POS = POS - 1;
      }
    }
  }
  if (REQUIRED && RET == 0) COMPILEERROR(CONCAT(CpLenPtr, 17, "Field not found: ", CpEnd | CpString, *NAME));
  RESULT = RET;
  return RESULT;
}
...
TPSTYPE *FINDFIELDTYPE(TPSTYPE *TYPEPTR, const PString *NAME, PBoolean REQUIRED) {
  TPSTYPE *RESULT;
  PInteger POS;
  POS = ({ PString tmp1 = *NAME; FINDFIELD(TYPEPTR, &tmp1, REQUIRED); });
...
TPSWITHVAR *FINDWITHVAR(const PString *NAME) {
  ...
  while (RET == (void*)0 && DEF != (void*)0 && DEF->CLS != TDCSCOPEBOUNDARY) {
    if (DEF->CLS == TDCWITHVAR) {
      ...
      if (({ PString tmp1 = *NAME; FINDFIELDTYPE(TYPEPTR, &tmp1, 0); }) != (void*)0) RET = DEF->WITHVARPTR;
    }
    DEF = DEF->PREV;
  }
  RESULT = RET;
...
TPSVARIABLE *ADDTMPVARIABLE(const PString *PREFIX, TPSTYPE *TYPEPTR) {
  ...
  RESULT = ADDVARIABLE(({ PString tmp1 = CONCAT(CpString, *PREFIX, CpEnd | CpStringPtr, &VARNUM); MAKEVARIABLE(&tmp1, TYPEPTR); }));
  return RESULT;
}
...
TPSVARIABLE MAKEARG(const PString *NAME, TPSTYPE *TYPEPTR) {
  TPSVARIABLE RESULT;
  RESULT = ({ PString tmp1 = *NAME; _MAKEARG(&tmp1, TYPEPTR, 0, 0); });
  return RESULT;
}

TPSVARIABLE MAKEVARARG(const PString *NAME, TPSTYPE *TYPEPTR) {
  TPSVARIABLE RESULT;
  RESULT = ({ PString tmp1 = *NAME; _MAKEARG(&tmp1, TYPEPTR, 1, 0); });
  return RESULT;
}

TPSVARIABLE MAKECONSTARG(const PString *NAME, TPSTYPE *TYPEPTR) {
  TPSVARIABLE RESULT;
  RESULT = ({ PString tmp1 = *NAME; _MAKEARG(&tmp1, TYPEPTR, 0, 1); });
  return RESULT;
}
...
TEXPRESSIONOBJ *EXSETADDRANGE(TEXPRESSIONOBJ *SETEXPR, TEXPRESSIONOBJ *FIRST, TEXPRESSIONOBJ *LAST) {
  ...
  if (LAST != (void*)0 && !ISSAMETYPE(GETFUNDAMENTALTYPE(LAST->TYPEPTR), ELEMENTTYPEPTR) && !ISSAMETYPE(FIRST->TYPEPTR, LAST->TYPEPTR)) {
...
void PSTYPEDCONSTANT(const PString *NAME) {
  ...
  TYPEPTR = PSTYPEDENOTER();
  WANTTOKENANDREAD(TKEQUALS);
  OUTCONSTANTDEFINITIONBEGIN(ADDVARIABLE(({ PString tmp1 = *NAME; MAKETYPEDCONSTANT(&tmp1, TYPEPTR); })));
  PSCONSTANTVALUE(TYPEPTR);
...
PString PARSESTRING(const PString *PSTR) {
  ...
  STATE = NONE;
  POS = 1;
  while (POS <= ({ PString tmp1 = *PSTR; LENGTH(&tmp1); })) {
    CH = (*PSTR).chr[POS];
    if (STATE == NONE) {
      POS = POS + 1;
      if (CH == '\'') {
    ...
    else if (STATE == ERROR) COMPILEERROR(CONCAT(CpLenPtr, 29, "Invalid character in string: ", CpEnd | CpString, *PSTR));
  }
  if (STATE == QUOTEDSTR || STATE == CARET) COMPILEERROR(CONCAT(CpLenPtr, 26, "String is not terminated: ", CpEnd | CpString, *PSTR));
...
PInteger PARSEINT(const PString *PSTR) {
  ...
  if (CODE != 0) COMPILEERROR(CONCAT(CpLenPtr, 25, "Could not parse integer: ", CpEnd | CpString, *PSTR));
  return RESULT;
}
...
PReal PARSEREAL(const PString *PSTR) {
  ...
  if (CODE != 0) COMPILEERROR(CONCAT(CpLenPtr, 29, "Could not parse real number: ", CpEnd | CpString, *PSTR));
  return RESULT;
}
...
void EXECUTEDIRECTIVE(const PString *DIR) {
  if (({ PString tmp1 = *DIR; LENGTH(&tmp1); }) > 3 && (*DIR).chr[2] == 'I' && (*DIR).chr[3] == ' ') LXINCLUDE(({ PString tmp2 = *DIR; COPY(&tmp2, 4, 255); }));
  else if (({ PString tmp3 = *DIR; LENGTH(&tmp3); }) == 3 && (*DIR).chr[2] == 'R' && ((*DIR).chr[3] == '+' || (*DIR).chr[3] == '-')) OPTIONS.CHECKBOUNDS = (*DIR).chr[3] == '+';
}
...
void STARTGLOBALSCOPE() {
  ...
  ADDFUNCTION(({ PString tmp123 = str_make(4, "HALT"); MAKEPROCEDURE1(&tmp123, ({ PString tmp122 = str_make(4, "CODE"); MAKEARG(&tmp122, PRIMITIVETYPES.PTINTEGER); })); }));
  ADDFUNCTION(({ PString tmp124 = str_make(10, "PARAMCOUNT"); MAKEFUNCTION0(&tmp124, PRIMITIVETYPES.PTINTEGER); }));
  ADDFUNCTION(({ PString tmp126 = str_make(8, "PARAMSTR"); MAKEFUNCTION1(&tmp126, PRIMITIVETYPES.PTSTRING, ({ PString tmp125 = str_of('I'); MAKEARG(&tmp125, PRIMITIVETYPES.PTINTEGER); })); }));
  ADDFUNCTION(({ PString tmp127 = str_make(8, "RANDOM_n"); MAKEFUNCTION0(&tmp127, PRIMITIVETYPES.PTREAL); }));
  ADDFUNCTION(({ PString tmp129 = str_make(8, "RANDOM_i"); MAKEFUNCTION1(&tmp129, PRIMITIVETYPES.PTINTEGER, ({ PString tmp128 = str_make(3, "NUM"); MAKEARG(&tmp128, PRIMITIVETYPES.PTINTEGER); })); }));
  ADDFUNCTION(({ PString tmp130 = str_make(9, "RANDOMIZE"); MAKEPROCEDURE0(&tmp130); }));
  DEF = DEFS.LATEST;
  while (DEF != (void*)0) {
    switch (DEF->CLS) {
      case TDCVARIABLE:
...
void _OUTCSTRING(const PString *STR) {
  ...
  do {
    PInteger first = 1;
    PInteger last = ({ PString tmp1 = *STR; LENGTH(&tmp1); });
    if (first <= last) {
      POS = first;
      while (1) {
        {
          ...
          if (CH < ' ' || CH > '~') {
            _OUTESCAPEDCHAR(CH);
            if (POS < ({ PString tmp2 = *STR; LENGTH(&tmp2); }) && LXISHEXDIGIT((*STR).chr[POS + 1])) WRITE(&CODEGEN.OUTPUT, RwpLenPtr | RwpEnd, 2, "\"\"");
          }
          else {
...
void _OUTSTRING(const PString *STR) {
  if (({ PString tmp1 = *STR; LENGTH(&tmp1); }) == 1) {
    WRITE(&CODEGEN.OUTPUT, RwpLenPtr | RwpEnd, 7, "str_of(");
    _OUTCHAR((*STR).chr[1]);
    WRITE(&CODEGEN.OUTPUT, RwpChar | RwpEnd, ')');
  }
  else {
    WRITE(&CODEGEN.OUTPUT, RwpLenPtr, 9, "str_make(", RwpInt | RwpEnd, ({ PString tmp2 = *STR; LENGTH(&tmp2); }));
    _OUTCOMMA();
    {
      PString tmp3 = *STR;
      _OUTCSTRING(&tmp3);
...
void OUTNAMEANDTYPE(const PString *NAME, TPSTYPE *TYPEPTR) {
  if (TYPEPTR == (void*)0) WRITE(&CODEGEN.OUTPUT, RwpLenPtr, 5, "void ", RwpString | RwpEnd, *NAME);
  else if (TYPEPTR->CLS == TTCPOINTER) {
    OUTTYPEREFERENCE(TYPEPTR->POINTEDTYPEPTR);
    WRITE(&CODEGEN.OUTPUT, RwpLenPtr, 2, " *", RwpString | RwpEnd, *NAME);
  }
  else if (TYPEPTR->ALIASFOR != (void*)0 && cmp_str(CoNotEq, CpStringPtr, &TYPEPTR->NAME, CpLenPtr, 0, "")) WRITE(&CODEGEN.OUTPUT, RwpStringPtr, &TYPEPTR->NAME, RwpChar, ' ', RwpString | RwpEnd, *NAME);
  else if (TYPEPTR->CLS == TTCBOOLEAN) WRITE(&CODEGEN.OUTPUT, RwpLenPtr, 9, "PBoolean ", RwpString | RwpEnd, *NAME);
  else if (TYPEPTR->CLS == TTCINTEGER) WRITE(&CODEGEN.OUTPUT, RwpLenPtr, 9, "PInteger ", RwpString | RwpEnd, *NAME);
  else if (TYPEPTR->CLS == TTCREAL) WRITE(&CODEGEN.OUTPUT, RwpLenPtr, 6, "PReal ", RwpString | RwpEnd, *NAME);
  else if (TYPEPTR->CLS == TTCCHAR) WRITE(&CODEGEN.OUTPUT, RwpLenPtr, 6, "PChar ", RwpString | RwpEnd, *NAME);
  else if (TYPEPTR->CLS == TTCSTRING) WRITE(&CODEGEN.OUTPUT, RwpLenPtr, 8, "PString ", RwpString | RwpEnd, *NAME);
  else if (TYPEPTR->CLS == TTCTEXT) WRITE(&CODEGEN.OUTPUT, RwpLenPtr, 6, "PFile ", RwpString | RwpEnd, *NAME);
  else if (TYPEPTR->CLS == TTCENUM) {
    PString tmp1 = *NAME;
    OUTNAMEANDENUM(&tmp1, TYPEPTR->ENUMPTR);
  }
  else if (TYPEPTR->CLS == TTCRANGE) {
    PString tmp2 = *NAME;
    OUTNAMEANDTYPE(&tmp2, GETFUNDAMENTALTYPE(TYPEPTR));
  }
    ...
    PString tmp3 = *NAME;
    OUTNAMEANDRECORD(&tmp3, TYPEPTR->RECPTR);
  }
  else if (TYPEPTR->CLS == TTCARRAY) {
    PString tmp4 = *NAME;
    OUTNAMEANDARRAY(&tmp4, TYPEPTR);
  }
  else INTERNALERROR(CONCAT(CpLenPtr, 29, "Error writing name and type: ", CpString, *NAME, CpLenPtr, 2, ", ", CpEnd | CpString, TYPENAME(TYPEPTR)));
}
...
PString _PF_FUN_OVERLOAD(const PString *NAMEPREFIX, TPSTYPE *TYPEPTR) {
  PString RESULT;
  TYPEPTR = GETFUNDAMENTALTYPE(TYPEPTR);
  if (ISBOOLEANTYPE(TYPEPTR)) RESULT = CONCAT(CpString, *NAMEPREFIX, CpEnd | CpLenPtr, 2, "_b");
  else if (ISINTEGERTYPE(TYPEPTR)) RESULT = CONCAT(CpString, *NAMEPREFIX, CpEnd | CpLenPtr, 2, "_i");
  else if (ISREALTYPE(TYPEPTR)) RESULT = CONCAT(CpString, *NAMEPREFIX, CpEnd | CpLenPtr, 2, "_r");
  else if (ISCHARTYPE(TYPEPTR)) RESULT = CONCAT(CpString, *NAMEPREFIX, CpEnd | CpLenPtr, 2, "_c");
  else if (ISSTRINGTYPE(TYPEPTR)) RESULT = CONCAT(CpString, *NAMEPREFIX, CpEnd | CpLenPtr, 2, "_s");
  else COMPILEERROR(CONCAT(CpLenPtr, 5, "Type ", CpString, TYPENAME(TYPEPTR), CpLenPtr, 26, " is not representable for ", CpLenPtr, 12, "overload of ", CpEnd | CpString, *NAMEPREFIX));
  return RESULT;
}
