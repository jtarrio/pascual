...
typedef struct record16 { int SIZE; struct record21 *VALUES[1 + 4 - 1]; } TEXFUNCTIONARGS;
...
typedef struct record25 { int SIZE; PString VALUES[1 + 128 - 1]; int ID; PBoolean HASBEENDEFINED; } TPSENUMDEF;
typedef struct record26 { PString NAME; struct record24 *TYPEINDEX; } TPSRECORDFIELD;
typedef struct record27 { int SIZE; TPSRECORDFIELD FIELDS[1 + 16 - 1]; int NUMVARIANTS; int VARIANTBOUNDS[1 + 16 - 1]; int ID; PBoolean HASBEENDEFINED; } TPSRECORDDEF;
...
typedef struct record31 { PString NAME; int ARGCOUNT; TPSVARIABLE ARGS[1 + 4 - 1]; struct record24 *RETURNTYPEINDEX; PBoolean ISDECLARATION; } TPSFUNCTION;
...
if (cmp_cc(CHR, '\'') == 0) {
INSTR = !INSTR;
if (INSTR && (POS > 1) && (cmp_cc((PSTR).chr[POS - 1], '\'') == 0)) STR = cat_sc(STR, '\'');
}
...
DISPOSEEXPR(&LHS);
 ...
 else if ((*(LHS)).CLS != XCNOTHING) if ((*(LHS)).CLS == XCBINARYOP) COMPILEERROR(cat_ss(str_make(17, "Invalid statement"), str_make(38, " (maybe you wrote \'=\' instead of \':=\'?")));
...
FUN.ARGCOUNT = 1;
FUN.ARGS[(1) - 1] = MAKEVARIABLE(str_make(3, "POS"), PRIMITIVETYPES.PTINTEGER, 1);
...
switch ((*(EXPR)).CLS) {
case XCNOTHING: COMPILEERROR(cat_ss(str_make(16, "Internal error: "), str_make(33, "trying to output empty expression")));
break;
