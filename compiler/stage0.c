...
typedef struct record8 { PString NAME; TPSTYPEINDEX ALIASFOR; TPSTYPECLASS CLS; union { struct { TPSENUMINDEX ENUMINDEX; }; struct { TPSRECORDINDEX RECORDINDEX; }; struct { TPSARRAYINDEX ARRAYINDEX; }; struct { TPSTYPEINDEX POINTEDTYPEINDEX; }; }; } TPSTYPE;
...
if (POS != 0) {
if ((DEFS.NAMES[(POS) - 1].CLS == TNCTYPE) && ISPLACEHOLDERTYPE(DEFS.NAMES[(POS) - 1].TYPEINDEX)) POS = DEFS.NAMES[(POS) - 1].TYPEINDEX;
...
ENUMPOS = first;
while (1) {
ADDNAME(MAKEORDINALNAME(DEFS.ENUMS[(TYP.ENUMINDEX) - 1].VALUES[(ENUMPOS) - 1], ENUMPOS, TNCENUMVALUE, POS));
...
{
RET = 0;
POS = DEFS.RECORDS[(DEFS.TYPES[(TYPEINDEX) - 1].RECORDINDEX) - 1].SIZE;
while ((POS >= 1) && (RET == 0)) {
if (cmp_ss(NAME, DEFS.RECORDS[(DEFS.TYPES[(TYPEINDEX) - 1].RECORDINDEX) - 1].FIELDS[(POS) - 1].NAME) == 0) RET = DEFS.RECORDS[(DEFS.TYPES[(TYPEINDEX) - 1].RECORDINDEX) - 1].FIELDS[(POS) - 1].TYPEINDEX;
POS = POS - 1;
}
}
...
TPSEXPRESSION EXPR;
{
if (DEFS.VARIABLES[(VARINDEX) - 1].ISREFERENCE) EXPR.VALUE = cat_cs('*', DEFS.VARIABLES[(VARINDEX) - 1].NAME);
 else EXPR.VALUE = DEFS.VARIABLES[(VARINDEX) - 1].NAME;
EXPR.CLS = TECVALUE;
...
TPSEXPRESSION EXPR;
{
EXPR.VALUE = DEFS.ENUMS[(DEFS.TYPES[(TYPEINDEX) - 1].ENUMINDEX) - 1].VALUES[(ORDINAL) - 1];
EXPR.CLS = TECVALUE;
...
{
EXPR.VALUE = cat_sc(cat_ss(cat_ss(cat_ss(cat_ss(BASE.VALUE, str_make(2, "[(")), IDX.VALUE), str_make(4, ") - ")), DEFS.ARRAYS[(DEFS.TYPES[(BASE.TYPEINDEX) - 1].ARRAYINDEX) - 1].LOWBOUND), ']');
EXPR.CLS = TECVALUE;
EXPR.TYPEINDEX = DEFS.ARRAYS[(DEFS.TYPES[(BASE.TYPEINDEX) - 1].ARRAYINDEX) - 1].TYPEINDEX;
EXPR.ISCONSTANT = BASE.ISCONSTANT;
}
}
...
if (!DEFS.RECORDS[(RECORDINDEX) - 1].HASBEENDEFINED) {
{
write_s(&CODEGEN.OUTPUT, str_make(3, " { "));
}
{
int first = 1;
int last = DEFS.RECORDS[(RECORDINDEX) - 1].SIZE;
if (first <= last) {
POS = first;
while (1) {
{
if ((DEFS.RECORDS[(RECORDINDEX) - 1].NUMVARIANTS > NUMVARIANT) && (DEFS.RECORDS[(RECORDINDEX) - 1].VARIANTBOUNDS[(NUMVARIANT + 1) - 1] == POS)) {
...
OUTNAMEANDTYPE(DEFS.RECORDS[(RECORDINDEX) - 1].FIELDS[(POS) - 1].NAME, DEFS.RECORDS[(RECORDINDEX) - 1].FIELDS[(POS) - 1].TYPEINDEX);
{
write_s(&CODEGEN.OUTPUT, str_make(2, "; "));
}
}
...
DEFS.RECORDS[(RECORDINDEX) - 1].HASBEENDEFINED = 1;
}
{
write_c(&CODEGEN.OUTPUT, ' ');
...
if (!DEFS.ENUMS[(ENUMINDEX) - 1].HASBEENDEFINED) {
{
write_s(&CODEGEN.OUTPUT, str_make(3, " { "));
}
{
int first = 1;
int last = DEFS.ENUMS[(ENUMINDEX) - 1].SIZE;
if (first <= last) {
POS = first;
while (1) {
{
...
{
write_s(&CODEGEN.OUTPUT, DEFS.ENUMS[(ENUMINDEX) - 1].VALUES[(POS) - 1]);
}
}
if (POS == last) break;
++POS;
...
DEFS.ENUMS[(ENUMINDEX) - 1].HASBEENDEFINED = 1;
}
{
write_c(&CODEGEN.OUTPUT, ' ');
write_s(&CODEGEN.OUTPUT, NAME);
