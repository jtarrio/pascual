          ...
          if (FNEXPR->FNPTR->ARGS[(int)subrange(POS, 1, 8) - 1].ISREFERENCE) {
            if (!FNCALL->CALLARGS.VALUES[(int)subrange(POS, 1, 8) - 1]->ISASSIGNABLE) {
              if (FNEXPR->FNPTR->ARGS[(int)subrange(POS, 1, 8) - 1].ISCONSTANT) {
                STR_i(DEFCOUNTER(TCTTMPVAR), &TMPVARNUM);
                RESULT = EXSETTMPVAR(EXVARIABLE(ADDVARIABLE(MAKEVARIABLE(cat_ss(str_make(3, "tmp"), TMPVARNUM), FNCALL->CALLARGS.VALUES[(int)subrange(POS, 1, 8) - 1]->TYPEPTR))), FNCALL->CALLARGS.VALUES[(int)subrange(POS, 1, 8) - 1], RESULT);
...
void PSRECORDVARIANTS(TPSRECORDDEF *REC) {
  ...
  do {
    ...
    do {
      ...
      if (CASELABEL->CLS != XCIMMEDIATE) COMPILEERROR(str_make(48, "The label of the case statement is not immediate"));
