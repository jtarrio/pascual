...
typedef enum enum4 { XCIMMEDIATE, XCTOSTRING, XCTOREAL, XCSUBRANGE, XCVARIABLE, XCFIELD, XCARRAY, XCPOINTER, XCSTRINGCHAR, XCFNREF, XCFNCALL, XCPSEUDOFNREF, XCPSEUDOFNCALL, XCUNARYOP, XCBINARYOP } TEXPRESSIONCLASS;
...
typedef enum enum7 { TCTENUM, TCTRECORD, TCTWITHVAR } TPSCOUNTERTYPE;
...
const char* enumvalues4[] = { "XCIMMEDIATE", "XCTOSTRING", "XCTOREAL", "XCSUBRANGE", "XCVARIABLE", "XCFIELD", "XCARRAY", "XCPOINTER", "XCSTRINGCHAR", "XCFNREF", "XCFNCALL", "XCPSEUDOFNREF", "XCPSEUDOFNCALL", "XCUNARYOP", "XCBINARYOP" };
...
const char* enumvalues7[] = { "TCTENUM", "TCTRECORD", "TCTWITHVAR" };
...
TPSVARIABLE MAKEARG(PString NAME, TPSTYPE *TYPEPTR, int ISREF) {
  TPSVARIABLE RESULT;
  RESULT.NAME = NAME;
  RESULT.TYPEPTR = TYPEPTR;
...
TEXPRESSIONOBJ *_EXUNOPIMM(TEXPRESSIONOBJ *PARENT, TLXTOKENID OP) {
  ...
  else if (OP == TKMINUS && PARENT->IMMEDIATE.CLS == XICREAL) PARENT->IMMEDIATE.REALVAL = -PARENT->IMMEDIATE.REALVAL;
  else if (OP == TKPLUS && PARENT->IMMEDIATE.CLS == XICREAL) ;
...
void STARTGLOBALSCOPE() {
  ...
  ADDFUNCTION(MAKEPROCEDURE2(str_make(6, "ASSIGN"), MAKEARG(str_of('F'), PRIMITIVETYPES.PTTEXT, 1), MAKEARG(str_make(4, "NAME"), PRIMITIVETYPES.PTSTRING, 0)));
  ADDFUNCTION(MAKEPROCEDURE1(str_make(5, "CLOSE"), MAKEARG(str_of('F'), PRIMITIVETYPES.PTTEXT, 1)));
  ADDFUNCTION(MAKEFUNCTION1(str_make(3, "EOF"), PRIMITIVETYPES.PTBOOLEAN, MAKEARG(str_of('F'), PRIMITIVETYPES.PTTEXT, 1)));
  ADDFUNCTION(MAKEPROCEDURE1(str_make(5, "RESET"), MAKEARG(str_of('F'), PRIMITIVETYPES.PTTEXT, 1)));
  ADDFUNCTION(MAKEPROCEDURE1(str_make(7, "REWRITE"), MAKEARG(str_of('F'), PRIMITIVETYPES.PTTEXT, 1)));
  ADDPSEUDOFN(str_make(6, "CONCAT"), TPFCONCAT);
  ADDPSEUDOFN(str_make(3, "STR"), TPFSTR);
  ADDPSEUDOFN(str_make(3, "VAL"), TPFVAL);
  ADDFUNCTION(MAKEFUNCTION1(str_make(3, "CHR"), PRIMITIVETYPES.PTCHAR, MAKEARG(str_make(3, "POS"), PRIMITIVETYPES.PTINTEGER, 0)));
  ADDFUNCTION(MAKEFUNCTION3(str_make(4, "COPY"), PRIMITIVETYPES.PTSTRING, MAKEARG(str_make(3, "STR"), PRIMITIVETYPES.PTSTRING, 0), MAKEARG(str_make(3, "POS"), PRIMITIVETYPES.PTINTEGER, 0), MAKEARG(str_make(3, "NUM"), PRIMITIVETYPES.PTINTEGER, 0)));
  ADDFUNCTION(MAKEPROCEDURE3(str_make(6, "DELETE"), MAKEARG(str_make(3, "STR"), PRIMITIVETYPES.PTSTRING, 1), MAKEARG(str_make(3, "POS"), PRIMITIVETYPES.PTINTEGER, 0), MAKEARG(str_make(3, "NUM"), PRIMITIVETYPES.PTINTEGER, 0)));
  ADDFUNCTION(MAKEPROCEDURE3(str_make(6, "INSERT"), MAKEARG(str_make(3, "INS"), PRIMITIVETYPES.PTSTRING, 0), MAKEARG(str_make(6, "TARGET"), PRIMITIVETYPES.PTSTRING, 1), MAKEARG(str_make(3, "POS"), PRIMITIVETYPES.PTINTEGER, 0)));
  ADDFUNCTION(MAKEFUNCTION1(str_make(6, "LENGTH"), PRIMITIVETYPES.PTINTEGER, MAKEARG(str_make(3, "STR"), PRIMITIVETYPES.PTSTRING, 0)));
  ADDFUNCTION(MAKEFUNCTION2(str_make(3, "POS"), PRIMITIVETYPES.PTINTEGER, MAKEARG(str_make(6, "NEEDLE"), PRIMITIVETYPES.PTSTRING, 0), MAKEARG(str_make(8, "HAYSTACK"), PRIMITIVETYPES.PTSTRING, 0)));
  ADDFUNCTION(MAKEFUNCTION1(str_make(6, "UPCASE"), PRIMITIVETYPES.PTCHAR, MAKEARG(str_make(3, "CHR"), PRIMITIVETYPES.PTCHAR, 0)));
  ADDFUNCTION(MAKEPROCEDURE1(str_make(4, "HALT"), MAKEARG(str_make(4, "CODE"), PRIMITIVETYPES.PTINTEGER, 0)));
  ADDFUNCTION(MAKEFUNCTION0(str_make(10, "PARAMCOUNT"), PRIMITIVETYPES.PTINTEGER));
  ADDFUNCTION(MAKEFUNCTION1(str_make(8, "PARAMSTR"), PRIMITIVETYPES.PTSTRING, MAKEARG(str_of('I'), PRIMITIVETYPES.PTINTEGER, 0)));
  DEF = DEFS.LATEST;
  while (DEF != (void*)0) {
    switch (DEF->CLS) {
      case TDCVARIABLE:
...
TEXPRESSIONOBJ *_GETLOWBOUND(TPSTYPE *TYPEPTR) {
  TEXPRESSIONOBJ *RESULT;
  if (ISRANGETYPE(TYPEPTR)) RESULT = PFORD(COPYEXPR(TYPEPTR->RANGEPTR->FIRST));
...
TEXPRESSIONOBJ *_GETHIGHBOUND(TPSTYPE *TYPEPTR) {
  ...
  else if (ISRANGETYPE(TYPEPTR)) RESULT = PFORD(COPYEXPR(TYPEPTR->RANGEPTR->LAST));
...
void _OUTVAL(TEXPRESSIONOBJ *EXPR) {
  ...
  if (ISENUMTYPE(DST->TYPEPTR)) {
    _OUTINDENT();
    WRITE_s(&CODEGEN.OUTPUT, str_make(7, "VAL_e(&"));
    _OUTEXPRESSIONPARENSPREC(SRC, 2);
    WRITE_s(&CODEGEN.OUTPUT, str_make(3, ", &"));
    _OUTEXPRESSIONPARENSPREC(DST, 2);
    WRITE_s(&CODEGEN.OUTPUT, str_make(2, ", "));
...
TEXPRESSIONOBJ *PFVAL_PARSE(TEXPRESSIONOBJ *FNEXPR) {
  ...
  CODE = PSEXPRESSION();
  WANTTOKENANDREAD(TKRPAREN);
  if (!SRC->ISASSIGNABLE || !ISSTRINGTYPE(SRC->TYPEPTR)) COMPILEERROR(str_make(47, "First argument for VAL is not a string variable"));
  if (!DEST->ISASSIGNABLE || DEST->ISCONSTANT) COMPILEERROR(str_make(41, "Second argument for VAL is not a variable"));
