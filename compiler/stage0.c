...
typedef struct record9 { int SIZE; PString VALUES[1 + 128 - 1]; } TPSENUMDEF;
typedef struct record10 { PString NAME; TPSTYPEINDEX TYPEINDEX; } TPSRECORDFIELD;
typedef struct record11 { int SIZE; TPSRECORDFIELD FIELDS[1 + 16 - 1]; int NUMVARIANTS; int VARIANTBOUNDS[1 + 16 - 1]; } TPSRECORDDEF;
...
typedef struct record16 { TPSEXPRESSION EXPR; } TPSWITHVAR;
...
typedef struct record19 { TPSDEFBOUNDS BASE; struct record19 *PREV; } TPSSCOPEELEMENT;
...
DEFS.WITHVARS[(DEFS.BOUNDS.WITHVARS) - 1].EXPR = BASE;
return_ADDWITHVAR = DEFS.BOUNDS.WITHVARS;
...
if (WITHVARINDEX != 0) EXPR = EXPRFIELDACCESS(DEFS.WITHVARS[(WITHVARINDEX) - 1].EXPR, ID.NAME);
 else {
