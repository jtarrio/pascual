...
typedef struct { int SIZE; STRING VALUES[1 + 64 - 1]; } TPSENUMDEF;
...
typedef struct { int NUMTYPES; int NUMENUMS; int NUMRECORDS; int NUMARRAYS; int NUMVARIABLES; int NUMFUNCTIONS; } TPSSCOPE;
typedef struct { TPSSCOPE SCOPE; TPSTYPE TYPES[1 + 64 - 1]; TPSENUMDEF ENUMS[1 + 16 - 1]; TPSRECORDDEF RECORDS[1 + 32 - 1]; TPSARRAYDEF ARRAYS[1 + 32 - 1]; TPSVARIABLE VARIABLES[1 + 32 - 1]; TPSFUNCTION FUNCTIONS[1 + 256 - 1]; } TPSDEFS;
...
{
if (POS != 1) RET = cat_sc(RET, ',');
RET = cat_ss(RET, DEEPTYPENAME(DEFS.RECORDS[(TYP.RECORDINDEX) - 1].FIELDS[(POS) - 1].TYPEINDEX, TRUE));
...
RET = cat_ss(cat_ss(RET, str_make(5, "] of ")), DEEPTYPENAME(DEFS.ARRAYS[(TYP.ARRAYINDEX) - 1].TYPEINDEX, TRUE));
...
return_TYPENAME = DEEPTYPENAME(TYPEINDEX, FALSE);
...
ADDVARIABLE(MAKEVARIABLE(str_make(5, "FALSE"), PRIMITIVETYPES.PTBOOLEAN, FALSE), GLOBALSCOPE);
ADDVARIABLE(MAKEVARIABLE(str_make(4, "TRUE"), PRIMITIVETYPES.PTBOOLEAN, FALSE), GLOBALSCOPE);
ADDVARIABLE(MAKEVARIABLE(str_make(5, "INPUT"), PRIMITIVETYPES.PTTEXT, FALSE), GLOBALSCOPE);
ADDVARIABLE(MAKEVARIABLE(str_make(6, "OUTPUT"), PRIMITIVETYPES.PTTEXT, FALSE), GLOBALSCOPE);
ADDVARIABLE(MAKEVARIABLE(str_make(6, "STDERR"), PRIMITIVETYPES.PTTEXT, FALSE), GLOBALSCOPE);
FUN.NAME = str_make(4, "COPY");
FUN.ARGCOUNT = 3;
FUN.ARGS[(1) - 1] = MAKEVARIABLE(str_make(3, "STR"), PRIMITIVETYPES.PTSTRING, FALSE);
FUN.ARGS[(2) - 1] = MAKEVARIABLE(str_make(3, "POS"), PRIMITIVETYPES.PTINTEGER, FALSE);
FUN.ARGS[(3) - 1] = MAKEVARIABLE(str_make(3, "NUM"), PRIMITIVETYPES.PTINTEGER, FALSE);
FUN.RETURNTYPEINDEX = PRIMITIVETYPES.PTSTRING;
ADDFUNCTION(FUN);
FUN.NAME = str_make(6, "DELETE");
FUN.ARGCOUNT = 3;
FUN.ARGS[(1) - 1] = MAKEVARIABLE(str_make(3, "STR"), PRIMITIVETYPES.PTSTRING, TRUE);
FUN.ARGS[(2) - 1] = MAKEVARIABLE(str_make(3, "POS"), PRIMITIVETYPES.PTINTEGER, FALSE);
FUN.ARGS[(3) - 1] = MAKEVARIABLE(str_make(3, "NUM"), PRIMITIVETYPES.PTINTEGER, FALSE);
...
FUN.ARGCOUNT = 1;
FUN.ARGS[(1) - 1] = MAKEVARIABLE(str_of('F'), PRIMITIVETYPES.PTTEXT, FALSE);
FUN.RETURNTYPEINDEX = PRIMITIVETYPES.PTBOOLEAN;
ADDFUNCTION(FUN);
FUN.NAME = str_make(4, "HALT");
FUN.ARGCOUNT = 1;
FUN.ARGS[(1) - 1] = MAKEVARIABLE(str_make(4, "CODE"), PRIMITIVETYPES.PTINTEGER, FALSE);
FUN.RETURNTYPEINDEX = 0;
ADDFUNCTION(FUN);
...
FUN.ARGCOUNT = 1;
FUN.ARGS[(1) - 1] = MAKEVARIABLE(str_make(3, "CHR"), PRIMITIVETYPES.PTCHAR, FALSE);
...
ENUM.VALUES[(ENUM.SIZE) - 1] = LXTOKEN.VALUE;
READTOKEN();
...
WANTTOKEN(TKIDENTIFIER);
REC.SIZE = REC.SIZE + 1;
REC.FIELDS[(REC.SIZE) - 1].NAME = LXTOKEN.VALUE;
READTOKEN();
...
TYPEINDEX = PSTYPEDENOTER(SCOPE);
WANTTOKENANDREAD(TKSEMICOLON);
OUTVARIABLEDEFINITION(ADDVARIABLE(MAKEVARIABLE(NAME, TYPEINDEX, FALSE), SCOPE));
} while (!(LXTOKEN.ID != TKIDENTIFIER));
OUTENUMVALUESINSCOPE(PREVIOUSSCOPE);
}
...
SKIPTOKEN(TKFORWARD);
WANTTOKENANDREAD(TKSEMICOLON);
DEFS.FUNCTIONS[(FNINDEX) - 1].ISDECLARATION = TRUE;
...
SKIPTOKEN(TKVAR);
WANTTOKEN(TKIDENTIFIER);
DEF.ARGS[(DEF.ARGCOUNT) - 1].NAME = LXTOKEN.VALUE;
READTOKEN();
WANTTOKENANDREAD(TKCOLON);
...
TPSEXPRESSION EXPR;
TPSIDENTIFIER ID;
if (LXTOKEN.ID == TKSTRING) {
EXPR = GENSTRINGCONSTANT(LXTOKEN.VALUE);
READTOKEN();
}
 else if (LXTOKEN.ID == TKNUMBER) {
...
TPSVARIABLE FIRST;
TPSVARIABLE LAST;
FIRST = MAKEVARIABLE(str_make(5, "first"), ID.TYPEINDEX, FALSE);
LAST = MAKEVARIABLE(str_make(4, "last"), ID.TYPEINDEX, FALSE);
{
write_c(OUTPUT, '{');
writeln(OUTPUT);
}
