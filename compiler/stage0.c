...
typedef enum __attribute__((__packed__)) enum3 { XCIMMEDIATE, XCTOSTRING, XCTOREAL, XCTORAWPTR, XCWITHTMPVAR, XCSUBRANGE, XCSET, XCVARIABLE, XCFIELD, XCARRAY, XCPOINTER, XCADDRESS, XCSTRINGCHAR, XCFNREF, XCFNCALL, XCPSEUDOFNREF, XCSIZEOF, XCCONVERTTOSTR, XCCONVERTTOVAL, XCREAD, XCWRITE, XCUNARYOP, XCBINARYOP } TEXPRESSIONCLASS;
typedef struct record11 {
  ...
  union {
    ...
    struct {
      struct record11* TORAWPTRPARENT;
    };
    struct {
...
typedef enum __attribute__((__packed__)) enum4 { TTCBOOLEAN, TTCINTEGER, TTCREAL, TTCCHAR, TTCSTRING, TTCTEXT, TTCENUM, TTCRANGE, TTCSET, TTCRECORD, TTCARRAY, TTCPOINTER, TTCNIL, TTCPOINTERUNKNOWN, TTCRAWPTR, TTCFUNCTION } TPSTYPECLASS;
...
const char* enumvalues3[] = { "XCIMMEDIATE", "XCTOSTRING", "XCTOREAL", "XCTORAWPTR", "XCWITHTMPVAR", "XCSUBRANGE", "XCSET", "XCVARIABLE", "XCFIELD", "XCARRAY", "XCPOINTER", "XCADDRESS", "XCSTRINGCHAR", "XCFNREF", "XCFNCALL", "XCPSEUDOFNREF", "XCSIZEOF", "XCCONVERTTOSTR", "XCCONVERTTOVAL", "XCREAD", "XCWRITE", "XCUNARYOP", "XCBINARYOP" };
const char* enumvalues4[] = { "TTCBOOLEAN", "TTCINTEGER", "TTCREAL", "TTCCHAR", "TTCSTRING", "TTCTEXT", "TTCENUM", "TTCRANGE", "TTCSET", "TTCRECORD", "TTCARRAY", "TTCPOINTER", "TTCNIL", "TTCPOINTERUNKNOWN", "TTCRAWPTR", "TTCFUNCTION" };
...
PBoolean ISRAWPTRTYPE(TPSTYPE* TYPEPTR) {
  PBoolean RESULT;
  RESULT = _TYPEHASCLASS(TYPEPTR, TTCRAWPTR);
  return RESULT;
}
...
PString DEEPTYPENAME(TPSTYPE* TYPEPTR, PBoolean USEORIGINAL) {
        ...
        while (1) {
          {
            ...
            RESULT = CONCAT(CpStringPtr, &RESULT, CpString, DEEPTYPENAME(TYP.RECPTR->FIELDS[subrange(POS, 1, 64) - 1].TYPEPTR, 0), CpChar, ':', CpEnd | CpStringPtr, &TYP.RECPTR->FIELDS[subrange(POS, 1, 64) - 1].NAME);
          }
          if (POS == last) break;
          ++POS;
        }
  ...
  else if (TYP.CLS == TTCPOINTER) RESULT = CONCAT(CpChar, '^', CpEnd | CpString, DEEPTYPENAME(TYP.POINTEDTYPEPTR, USEORIGINAL));
...
TEXPRESSIONOBJ* EXCOPY(TEXPRESSIONOBJ* EXPR) {
  ...
  switch (EXPR->CLS) {
    ...
    case XCTOREAL:
      ...
      break;
    case XCTORAWPTR:
      COPY->TORAWPTRPARENT = EXCOPY(EXPR->TORAWPTRPARENT);
      break;
    case XCWITHTMPVAR:
      {
  ...
  if (RESULT < 0) {
    switch (EXPR->CLS) {
      ...
      case XCTOREAL:
        ...
        break;
      case XCTORAWPTR:
        RESULT = _EXPRPRECEDENCE(EXPR->TORAWPTRPARENT);
        break;
      case XCWITHTMPVAR:
...
PString EXDESCRIBE(TEXPRESSIONOBJ* EXPR) {
  ...
  switch (EXPR->CLS) {
    ...
    case XCTOREAL:
      ...
      break;
    case XCTORAWPTR:
      RESULT = EXDESCRIBE(EXPR->TORAWPTRPARENT);
      break;
    case XCWITHTMPVAR:
...
TEXPRESSIONOBJ* EXTORAWPTR(TEXPRESSIONOBJ* PARENT) {
  TEXPRESSIONOBJ* RESULT;
...
TEXPRESSIONOBJ* EXCOERCE(TEXPRESSIONOBJ* EXPR, TPSTYPE* TYPEPTR) {
  ...
  else if (ISPOINTERYTYPE(EXPR->TYPEPTR) && ISRAWPTRTYPE(TYPEPTR)) RESULT = EXTORAWPTR(EXPR);
...
void PSIDENTIFIERSTATEMENT() {
  ...
  if (LEXER.TOKEN.ID == TKASSIGN) {
    ...
    if (LHS->CLS == XCFNCALL || LHS->TYPEPTR == PNil) OUTPROCEDURECALL(LHS);
...
void STARTGLOBALSCOPE() {
  ...
  PRIMITIVETYPES.PTRAWPTR = ({ PString tmp8 = str_make(13, "{raw pointer}"); MAKEBASETYPE(&tmp8, TTCRAWPTR); });
  {
  ...
  {
    TPSFUNCTION tmp23 = ({ PString tmp22 = str_make(7, "Dispose"); MAKEPROCEDURE1(&tmp22, ({ PString tmp21 = str_make(3, "PTR"); MAKEVARARG(&tmp21, PRIMITIVETYPES.PTRAWPTR); })); });
    ADDFUNCTION(&tmp23);
  }
  {
    TPSFUNCTION tmp27 = ({ PString tmp26 = str_make(3, "New"); MAKEPROCEDURE2(&tmp26, ({ PString tmp24 = str_make(3, "PTR"); MAKEVARARG(&tmp24, PRIMITIVETYPES.PTRAWPTR); }), ({ PString tmp25 = str_make(4, "SIZE"); MAKEARG(&tmp25, PRIMITIVETYPES.PTINTEGER); })); });
    ADDFUNCTION(&tmp27);
  }
  REGISTERGLOBALS_IO();
...
void OUTEXPRESSION(TEXPRESSIONOBJ* EXPR) {
  switch (EXPR->CLS) {
    ...
    case XCTOREAL:
      ...
      break;
    case XCTORAWPTR:
      OUTEXPRESSION(EXPR->TORAWPTRPARENT);
      break;
    case XCWITHTMPVAR:
...
void OUTTYPEREFERENCE(TPSTYPE* TYPEPTR) {
  ...
  else if (TYPEPTR->CLS == TTCRAWPTR) Write(&CODEGEN.OUTPUT, 1, RwpLenPtr | RwpEnd, 5, "void*");
...
void OUTNAMEANDTYPE(const PString *NAME, TPSTYPE* TYPEPTR) {
  ...
  else if (TYPEPTR->CLS == TTCRAWPTR) Write(&CODEGEN.OUTPUT, 1, RwpLenPtr, 6, "void* ", RwpStringPtr | RwpEnd, NAME);
