...
TSDTMPVARDEF* GETTEMPORARYVARIABLE(TSDTYPEDEF* THETYPEPTR, PBoolean WANTREFERENCE) {
  TSDTMPVARDEF* RESULT;
  TSDEFENTRY* DEF;
  TSDVARIABLEDEF* tmp1;
  ...
  tmp1 = &RESULT->VARDEF;
  tmp1->NAME = CONCAT(CpLenPtr, 3, "tmp", CpEnd | CpString, INTTOSTR(DEFCOUNTER(SCTTMPVAR)));
  tmp1->TYPEPTR = THETYPEPTR;
  tmp1->ISCONSTANT = 0;
  tmp1->CONSTANTVALUE = PNil;
  tmp1->ISREFERENCE = WANTREFERENCE;
  tmp1->LOCATION = PNil;
  tmp1->ISARGUMENT = 0;
  tmp1->WASINITIALIZED = 1;
  tmp1->WASUSED = 1;
  tmp1->ISALIASFOR = PNil;
  return RESULT;
}
