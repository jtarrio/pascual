...
PString PF_DESCRIBECALL(TEXPRESSIONOBJ *EXPR, int LEVELS);
...
PString _DESCRIBEWITHTMPVAR(TEXPRESSIONOBJ *EXPR, int LEVELS) {
  PString RESULT;
...
PString _DESCRIBESET(TEXPRESSIONOBJ *EXPR, int LEVELS) {
  PString RESULT;
...
TEXSETIMMBOUNDSOBJ *EXSETADDBOUNDS(TEXSETIMMBOUNDSOBJ *BOUNDS, int FIRST, int LAST) {
...
int _EXISZERO(TEXPRESSIONOBJ *EXPR) {
  int RESULT;
  RESULT = EXISIMMEDIATEOFCLASS(EXPR, XICINTEGER) && EXPR->IMMEDIATE.INTEGERVAL == 0 || EXISIMMEDIATEOFCLASS(EXPR, XICREAL) && (double)EXPR->IMMEDIATE.INTEGERVAL ==   0.0000000000E+00;
  return RESULT;
}

int _EXISONE(TEXPRESSIONOBJ *EXPR) {
  int RESULT;
  RESULT = EXISIMMEDIATEOFCLASS(EXPR, XICINTEGER) && EXPR->IMMEDIATE.INTEGERVAL == 1 || EXISIMMEDIATEOFCLASS(EXPR, XICREAL) && (double)EXPR->IMMEDIATE.INTEGERVAL ==   1.0000000000E+00;
  return RESULT;
}
...
void STARTGLOBALSCOPE() {
  ...
  ADDCONSTANT(MAKECONSTANT(str_make(2, "PI"), EXREALCONSTANT(  3.1415926536E+00)));
...
void _OUTEXSET(TEXPRESSIONOBJ *EXPR) {
  ...
  WRITE_s(&CODEGEN.OUTPUT, str_make(7, " dst = "));
  _OUTSETIMMEDIATE(EXPR->SETBASE);
  WRITE_s(&CODEGEN.OUTPUT, str_make(2, "; "));
  BOUNDS = EXPR->SETBOUNDS;
  while (BOUNDS != (void*)0) {
    WRITE_s(&CODEGEN.OUTPUT, str_make(8, "set_set("));
    ...
    WRITE_s(&CODEGEN.OUTPUT, str_make(13, ", dst.bits); "));
    DISPOSEEXPR(&FIRST);
    DISPOSEEXPR(&LAST);
    BOUNDS = BOUNDS->NEXT;
  }
...
void _OUTEXSETOPERATION(TEXPRESSIONOBJ *LEFT, TEXPRESSIONOBJ *RIGHT, TLXTOKENID OP) {
  ...
  if (OP == TKLESSOREQUALS) _OUTEXSETOPERATION(RIGHT, LEFT, TKMOREOREQUALS);
    ...
    if (OP == TKEQUALS) {
      ...
      WRITE_s(&CODEGEN.OUTPUT, str_make(15, "set_issuperset("));
      _OUTEXPRESSIONPARENSPREC(LEFT, 1);
      WRITE_s(&CODEGEN.OUTPUT, str_make(7, ".bits, "));
      _OUTEXPRESSIONPARENSPREC(RIGHT, 1);
      WRITE_s(&CODEGEN.OUTPUT, str_make(7, ".bits, "));
