...
typedef enum __attribute__((__packed__)) enum4 { XCIMMEDIATE, XCTOSTRING, XCTOREAL, XCWITHTMPVAR, XCSUBRANGE, XCSET, XCVARIABLE, XCFIELD, XCARRAY, XCPOINTER, XCADDRESS, XCSTRINGCHAR, XCFNREF, XCFNCALL, XCPSEUDOFNREF, XCPSEUDOFNCALL, XCUNARYOP, XCBINARYOP } TEXPRESSIONCLASS;
...
void STARTGLOBALSCOPE() {
  ...
  ADDFUNCTION(({ PString tmp120 = str_make(9, "LOWERCASE"); MAKEFUNCTION1(&tmp120, PRIMITIVETYPES.PTCHAR, ({ PString tmp119 = str_make(3, "CHR"); MAKEARG(&tmp119, PRIMITIVETYPES.PTCHAR); })); }));
