...
typedef enum enum5 { TPFDISPOSE, TPFNEW, TPFORD, TPFPRED, TPFREAD, TPFREADLN, TPFSTR, TPFSUCC, TPFVAL, TPFWRITE, TPFWRITELN } TPSPSEUDOFN;
...
const char* enumvalues5[] = { "TPFDISPOSE", "TPFNEW", "TPFORD", "TPFPRED", "TPFREAD", "TPFREADLN", "TPFSTR", "TPFSUCC", "TPFVAL", "TPFWRITE", "TPFWRITELN" };
...
void PSFUNCTIONBODY(TPSFUNCTION *FNPTR) {
  ...
  if (FNPTR->RETURNTYPEPTR != (void*)0) {
    RESULTPTR = ADDVARIABLE(MAKEVARIABLE(str_make(6, "RESULT"), FNPTR->RETURNTYPEPTR, 0));
    RESULTPTR->WASUSED = 1;
...
TEXPRESSIONOBJ *PFORD(TEXPRESSIONOBJ *ARG) {
  TEXPRESSIONOBJ *RESULT;
  TEXIMMEDIATE IMM;
  if (!ISORDINALTYPE(ARG->TYPEPTR)) COMPILEERROR(str_make(38, "Argument does not have an ordinal type"));
...
TEXPRESSIONOBJ *PFSTR_PARSE(TEXPRESSIONOBJ *FNEXPR) {
  ...
  if (!ISBOOLEANTYPE(SRC->TYPEPTR) && !ISINTEGERTYPE(SRC->TYPEPTR) && !ISREALTYPE(SRC->TYPEPTR) && !ISENUMTYPE(SRC->TYPEPTR)) COMPILEERROR(cat_ss(str_make(37, "Source argument has an invalid type: "), TYPENAME(SRC->TYPEPTR)));
...
TEXPRESSIONOBJ *PFVAL_PARSE(TEXPRESSIONOBJ *FNEXPR) {
  ...
  CODE = PSEXPRESSION();
  WANTTOKENANDREAD(TKRPAREN);
  if (!SRC->ISASSIGNABLE || !ISSTRINGTYPE(SRC->TYPEPTR)) COMPILEERROR(str_make(40, "Source argument is not a string variable"));
  if (!DEST->ISASSIGNABLE || DEST->ISCONSTANT) COMPILEERROR(str_make(38, "Destination argument is not a variable"));
  if (!ISBOOLEANTYPE(DEST->TYPEPTR) && !ISINTEGERTYPE(DEST->TYPEPTR) && !ISREALTYPE(DEST->TYPEPTR) && !ISENUMTYPE(DEST->TYPEPTR)) COMPILEERROR(cat_ss(str_make(42, "Destination argument has an invalid type: "), TYPENAME(DEST->TYPEPTR)));
  if (!CODE->ISASSIGNABLE || !ISINTEGERTYPE(CODE->TYPEPTR)) COMPILEERROR(str_make(40, "Code argument is not an integer variable"));
  EXMARKINITIALIZED(DEST);
